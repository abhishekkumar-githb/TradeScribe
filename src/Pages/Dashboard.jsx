  ...
  return (
    <div className="min-h-screen bg-slate-900 relative overflow-hidden">
      ...
      <div className="flex min-h-screen relative">
        {/* Sidebar */}
        <aside
          className={`${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"} transition-all duration-300 w-64 bg-slate-900/90 border-r border-slate-800 flex flex-col h-screen fixed z-50 md:static md:translate-x-0 backdrop-blur-xl`}
        >
            ...
            <div className="p-4">
              <div className="space-y-1">
                {dashboardConfig.generalItems.map((item, index) => (
                  <div key={index}>
                    <div
                      className="flex items-center text-sm py-3 px-4 cursor-pointer rounded-lg transition-colors duration-300 text-slate-300 hover:bg-slate-800 hover:text-white"
                    ...
                    {openSubmenus[index] && item.sublabels?.length > 0 && (
                      <div className="ml-6 mt-2 space-y-1">
                        {item.sublabels.map((sublabel, subIndex) => (
                          <div
                            key={subIndex}
                            className="flex items-center text-sm py-2 px-4 cursor-pointer rounded-lg transition-colors duration-300 text-slate-400 hover:bg-slate-800 hover:text-white"
                            onClick={() => navigate(`/dashboard${sublabel.path}`)}
                          >
                            <sublabel.icon className="w-4 h-4 mr-2 text-violet-400" />
        ...
        <main className="flex-1 overflow-y-auto h-screen scrollbar-hide relative">
          <div className="flex items-center justify-between bg-slate-900 border-b border-slate-800 p-4 fixed top-0 left-0 right-0 z-40 md:hidden">
